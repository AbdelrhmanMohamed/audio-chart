...
const AudiologyContext = createContext<AudiologyContextType | undefined>(
  undefined
);
...
const initialData: AudiogramData = {
  rightEar: [
    ...
    { id: "r2", frequency: 500, threshold: 80, testType: "BC" },
  ...
  leftEar: [
    ...
    { id: "l2", frequency: 500, threshold: 25, testType: "SF" },
...
export function AudiologyProvider({ children }: { children: React.ReactNode }) {
  ...
  const addAudiogramPoint = (
    ear: "right" | "left",
    newPoint: Omit<AudiogramPoint, "id">
  ) => {
    setData((prev) => {
      ...
      return {
        ...prev,
        [earKey]: [...prev[earKey], updatedPoint].sort(
  ...
  const updateAudiogramPoint = (
    ear: "right" | "left",
    updatedPoint: AudiogramPoint
  ) => {
    setData((prev) => {
      const earKey = ear === "right" ? "rightEar" : "leftEar";
      return {
  ...
  const updateSpeechAudiometry = (result: SpeechAudiometryResult) => {
    setData((prev) => ({
      ...prev,
      speechAudiometry: prev.speechAudiometry.map((item) =>
        item.ear === result.ear ? result : item
      ),
    }));
  };
...
export const useAudiology = () => {
  ...
  if (!context)
    throw new Error("useAudiology must be used within AudiologyProvider");
  return context;
};
